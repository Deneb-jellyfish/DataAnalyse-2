...
def write_overall_metrics() -> pd.DataFrame:
    ...
    overall.to_csv(OUTPUT_DIR / "overall_metrics_summary.csv", index=False)
    return overall
...
def write_registry() -> pd.DataFrame:
    ...
    for relative_path in EXPECTED_ARTIFACTS:
        ...
        rows.append(
            {
                "path": f"outputs/hourly/{relative_path}",
                "exists": path.exists(),
                "size_bytes": path.stat().st_size if path.exists() else 0,
            }
        )
    df = pd.DataFrame(rows)
    df.to_csv(OUTPUT_DIR / "experiment_registry.csv", index=False)
    return df
