...
def run_h1(model: ARIMAHourlyModel, test_pm25: np.ndarray, test_dti: pd.DatetimeIndex) -> list[dict]:
    ...
    pred_df.to_csv(OUTPUT_DIR / "arima_predictions_h1.csv", index=False)
    with open(OUTPUT_DIR / "arima_best_order_h1.json", "w", encoding="utf-8") as handle:
    ...
    log(
        ...
        f"path={OUTPUT_DIR / 'arima_predictions_h1.csv'}"
    )
...
def run_seq6(model: ARIMAHourlyModel, test_pm25: np.ndarray, test_dti: pd.DatetimeIndex) -> list[dict]:
    ...
    pred_df.to_csv(OUTPUT_DIR / "arima_predictions_seq6.csv", index=False)
    with open(OUTPUT_DIR / "arima_best_order_seq6.json", "w", encoding="utf-8") as handle:
        json.dump({"order": list(ARIMA_ORDER), "seasonal_order": list(ARIMA_SEASONAL)}, handle, ensure_ascii=False, indent=2)
    log(
        f"seq6 outputs saved: rows={len(pred_df)}, "
        f"path={OUTPUT_DIR / 'arima_predictions_seq6.csv'}"
    )
    return metric_rows_from_predictions(
        pred_df,
        model_name="ARIMA",
