...
def run_feature_ablation() -> None:
    ...
    for task, horizons, params_path in [
        ("h1", [1], resolve_artifact_path("xgboost_h1_best_params.json")),
        ("seq6", [1, 2, 3, 4, 5, 6], resolve_artifact_path("xgboost_seq6_best_params.json")),
    ]:
    ...
    pd.DataFrame(rows).to_csv(artifact_path("feature_ablation.csv", ensure_parent=True), index=False)
...
def run_error_analyses() -> None:
    ...
    pd.DataFrame(aqi_rows).to_csv(artifact_path("aqi_bucket_metrics.csv", ensure_parent=True), index=False)
    ...
    pd.DataFrame(hp_rows).to_csv(artifact_path("high_pollution_error_analysis.csv", ensure_parent=True), index=False)
    ...
    pd.DataFrame(hour_rows).to_csv(artifact_path("hourly_error_by_hour.csv", ensure_parent=True), index=False)
    ...
    pd.DataFrame(dp_rows).to_csv(artifact_path("hourly_error_by_dayperiod.csv", ensure_parent=True), index=False)
