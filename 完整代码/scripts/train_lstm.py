...
def run_task(task: str, profile: str = "full") -> tuple[list[dict], dict]:
    ...
    if output_len == 1:
        ...
        pred_path = artifact_path("lstm_predictions_h1.csv", ensure_parent=True)
        config_path = artifact_path("lstm_h1_config.json", ensure_parent=True)
        model_path = artifact_path("lstm_h1.pkl", ensure_parent=True)
        task_name = "h1"
    else:
        ...
        pred_path = artifact_path("lstm_predictions_seq6.csv", ensure_parent=True)
        config_path = artifact_path("lstm_seq6_config.json", ensure_parent=True)
        model_path = artifact_path("lstm_seq6.pkl", ensure_parent=True)
        task_name = "seq6"
...
def save_metrics(rows: list[dict]) -> None:
    ...
    path = artifact_path("lstm_metrics.csv", ensure_parent=True)
...
def save_train_history(all_histories: dict[str, dict]) -> None:
    ...
    path = artifact_path("lstm_train_history.json", ensure_parent=True)
    serializable = {
        task: {
