...
def run_task(task: str, batch_size: int, epochs: int, patience: int, lr: float) -> tuple[list[dict], TransformerTrainer]:
    ...
    if output_len == 1:
        ...
        pred_df.to_csv(artifact_path("transformer_predictions_h1.csv", ensure_parent=True), index=False)
        val_df.to_csv(artifact_path("transformer_predictions_h1_val.csv", ensure_parent=True), index=False)
        config_path = artifact_path("transformer_h1_config.json", ensure_parent=True)
        model_path = artifact_path("transformer_h1.pt", ensure_parent=True)
        task_name = "h1"
        log(
        ...
        pred_df.to_csv(artifact_path("transformer_predictions_seq6.csv", ensure_parent=True), index=False)
        val_df.to_csv(artifact_path("transformer_predictions_seq6_val.csv", ensure_parent=True), index=False)
        config_path = artifact_path("transformer_seq6_config.json", ensure_parent=True)
        model_path = artifact_path("transformer_seq6.pt", ensure_parent=True)
        task_name = "seq6"
        log(
            f"saved prediction files for task={task}: "
            f"test_rows={len(pred_df)}, val_rows={len(val_df)}"
...
def save_metrics(rows: list[dict]) -> None:
    ...
    path = artifact_path("transformer_metrics.csv", ensure_parent=True)
...
def save_history(all_trainers: list[TransformerTrainer], tasks: list[str]) -> None:
    ...
    path = artifact_path("transformer_train_history.json", ensure_parent=True)
    history_data = {
        task: {
