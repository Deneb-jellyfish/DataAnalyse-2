...
def inject_css() -> None:
    st.markdown(
        ...
        /* multiselect 已选标签 */
        [data-baseweb="tag"] {
            background-color: rgba(212, 175, 90, 0.22) !important;
            border: 1px solid rgba(180, 140, 55, 0.38) !important;
        }
        [data-baseweb="tag"] span {
            color: #6b4c18 !important;
        }
        [data-baseweb="tag"] [role="button"] svg {
            fill: #8a6322 !important;
        }
        /* slider 轨道与滑块 */
        [data-baseweb="slider"] [role="slider"] {
            background-color: #c8a252 !important;
            border-color: #c8a252 !important;
        }
        [data-baseweb="slider-inner-track"] {
            background-color: #c8a252 !important;
        }
...
def load_overall_metrics() -> pd.DataFrame:
    df = load_csv(resolve_artifact_path("overall_metrics_summary.csv"))
    if df.empty:
        return df
...
def load_predictions() -> pd.DataFrame:
    ...
    for filename in files:
        df = load_csv(resolve_artifact_path(filename))
        if df.empty:
            continue
...
def load_realtime_assets():
    ...
    with open(resolve_artifact_path("lstm_h1_config.json"), encoding="utf-8") as handle:
    ...
    lstm_trainer = LSTMTrainer.load(resolve_artifact_path("lstm_h1.pkl"), model=lstm_model)
    lstm_trainer.model.eval()

    transformer_checkpoint = torch.load(
        resolve_artifact_path("transformer_seq6.pt"),
        map_location="cpu",
        weights_only=False,
    )
...
aqi_bucket = load_csv(resolve_artifact_path("aqi_bucket_metrics.csv"))
feature_ablation = load_csv(resolve_artifact_path("feature_ablation.csv"))
high_pollution = load_csv(resolve_artifact_path("high_pollution_error_analysis.csv"))
hourly_error = load_csv(resolve_artifact_path("hourly_error_by_hour.csv"))
dayperiod_error = load_csv(resolve_artifact_path("hourly_error_by_dayperiod.csv"))
cross_city = load_csv(resolve_artifact_path("cross_city_metrics.csv"))
registry = load_csv(resolve_artifact_path("experiment_registry.csv"))
...
if current_page == "实验总览":
    ...
    with conc1:
        render_panel(
            "h1 结论",
            f"当前 h1 单步预测中，<b>{best_h1['model']}</b> 表现最好，RMSE 为 {best_h1['rmse']:.2f}，MAE 为 {best_h1['mae']:.2f}。"
            if best_h1 is not None
            else "当前暂无 h1 指标。",
        )
    with conc2:
        render_panel(
            "6 小时序列结论",
            f"未来 6 小时逐小时预测中，<b>{best_seq6['model']}</b> 的平均表现最优，RMSE 为 {best_seq6['rmse']:.2f}。"
            if best_seq6 is not None
            else "当前暂无 6 小时序列指标。",
        )
    with conc3:
    ...
    tab_rmse, tab_mae, tab_mape = st.tabs(["RMSE 对比", "MAE 对比", "MAPE 对比"])

    _best_h1_name = best_h1["model"] if best_h1 is not None else "—"
    _best_h1_rmse = f"{best_h1['rmse']:.2f}" if best_h1 is not None else "—"
    _best_s6_name = best_seq6["model"] if best_seq6 is not None else "—"
    _best_s6_rmse = f"{best_seq6['rmse']:.2f}" if best_seq6 is not None else "—"

    with tab_rmse:
        img_col, desc_col = st.columns([1.65, 0.35])
        with img_col:
            show_saved_figure("fig07_overall_rmse.png", "图 3a  Overall RMSE comparison")
        with desc_col:
            render_panel(
                "RMSE 解读",
                f"RMSE 对大误差最敏感，适合衡量峰值污染情形下的模型风险。<br/><br/>"
                f"h1 最优：<b>{_best_h1_name}</b>，RMSE = {_best_h1_rmse}。<br/>"
                f"seq6 均值最优：<b>{_best_s6_name}</b>，RMSE = {_best_s6_rmse}。<br/><br/>"
                "多步任务误差随步长单调增加，h6 约为 h1 的 3× 水平。",
            )
    ...
    with tab_mae:
        ...
        with desc_col:
            render_panel(
                "MAE 解读",
                "MAE 对每个预测点等权，直观反映日常预测的平均偏差幅度，不被极端值主导。<br/><br/>"
                "模型排名与 RMSE 基本一致，说明误差分布相对均匀，没有特别集中的极端错误。<br/><br/>"
                "h1 任务的 MAE 普遍低于 10，预测精度在实际应用中可接受。",
            )
    ...
    with tab_mape:
        ...
        with desc_col:
            render_panel(
                "MAPE 解读",
                "MAPE 衡量相对误差，对低浓度样本（PM2.5 < 10）极度敏感，分母趋近于零时数值会急剧放大。<br/><br/>"
                "使用 MAPE 时需结合 AQI 分层结果判读，避免被少数低浓度异常样本拉高整体误差。<br/><br/>"
                "中高污染区间（PM2.5 > 75）的 MAPE 更具参考价值。",
            )
    ...
    with col_b:
        ...
        with tabs[0]:
            show_saved_figure("fig10_shap_h1.png", "图 4  XGBoost SHAP feature importance for h1")
        with tabs[1]:
            show_saved_figure("fig11_shap_seq6.png", "图 5  XGBoost SHAP feature importance for seq6")
        with tabs[2]:
    ...
    with ctrl2:
        h1_max_pts = st.slider(
            "展示最近 N 个预测点",
            min_value=48, max_value=360, value=168, step=24,
            key="h1_pts",
        )

    # ── 动态预测曲线 ──────────────────────────────────────────
    render_section_header("交互图", "h1 预测曲线（动态）", "根据上方选择的模型和时间跨度实时生成，展示最近测试集窗口。")
    if not predictions.empty and h1_sel_models:
        h1_pred_df = predictions[predictions["task"] == "h1"]
        st.pyplot(draw_prediction_curve(h1_pred_df, h1_sel_models, horizon=1, max_points=h1_max_pts), use_container_width=True)
    else:
        st.info("请至少选择一个模型。")

    # ── 预存成图 ──────────────────────────────────────────────
    with st.expander("预存图：RMSE 排名 & 残差分布", expanded=True):
        col_a, col_b = st.columns(2)
        with col_a:
            show_saved_figure("fig02_h1_model_rmse.png", "图 1b  h1 model RMSE comparison")
        with col_b:
            show_saved_figure("fig03_h1_scatter_grid.png", "图 1c  h1 scatter grid")

    # ── 指标表 ────────────────────────────────────────────────
    st.markdown("### h1 指标表")
    metric_view = h1_metrics.copy()
    if h1_sel_models:
        metric_view = metric_view[metric_view["model"].isin(h1_sel_models)]
    ...
    # ── 交互控件 ──────────────────────────────────────────────
    ctrl1, ctrl2, ctrl3 = st.columns([2.5, 0.8, 1])
    with ctrl1:
        s6_sel_models = st.multiselect(
            "选择展示的模型",
            options=all_models,
            default=all_models,
            key="seq6_model_sel",
    ...
    # ── 动态图：预测曲线 + RMSE 随步长趋势 ────────────────────
    render_section_header("交互图", f"seq6 预测曲线（h{s6_horizon}）& RMSE 随步长变化", "左侧展示所选步长的预测曲线，右侧实时更新各模型的逐步 RMSE 趋势。")
    curve_col, trend_col = st.columns([1.35, 0.65])
    with curve_col:
        if not predictions.empty and s6_sel_models:
            s6_pred_df = predictions[predictions["task"] == "seq6"]
            st.pyplot(draw_prediction_curve(s6_pred_df, s6_sel_models, horizon=s6_horizon, max_points=s6_max_pts), use_container_width=True)
        else:
            st.info("请至少选择一个模型。")
    with trend_col:
        if not seq6_metrics.empty and s6_sel_models:
            s6_trend_df = seq6_metrics[seq6_metrics["model"].isin(s6_sel_models)]
            st.pyplot(draw_seq6_curve(s6_trend_df, s6_sel_models), use_container_width=True)

    # ── 预存成图 ──────────────────────────────────────────────
    with st.expander("预存图：seq6 均值 RMSE 对比", expanded=False):
    ...
    if s6_sel_models:
        step_table = step_table[step_table["model"].isin(s6_sel_models)]
    ...
    mean_table = seq6_mean.copy()
    if s6_sel_models:
        mean_table = mean_table[mean_table["model"].isin(s6_sel_models)]
    st.dataframe(format_metric_table(mean_table), use_container_width=True, hide_index=True)
    ...
    if st.button("开始预测", type="primary"):
        with st.spinner("正在加载模型并推理，首次运行约需 10–30 秒，请稍候…"):
            try:
                summary, detail = run_realtime_prediction(edited)
            except Exception as exc:
                import traceback
                result_placeholder.error(f"预测失败：{exc}\n\n```\n{traceback.format_exc()}\n```")
                st.stop()
